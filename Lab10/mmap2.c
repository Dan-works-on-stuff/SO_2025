...
int main(int argc, char *argv[]) {
   ...
   în fișier, căci sunt peste EOF (deci nu se face append de informații la fișier) !!!   */
    ...
    map_addr = mmap(
        NULL, // Se va crea o mapare începând de la o adresă page-aligned aleasă de kernel (și returnată în map_addr)
        adjusted_length, // Lungimea aliniată (i.e. ajustată page-aligned) a mapării (calculată mai sus)
        PROT_READ | PROT_WRITE,
        // Tipul de protecție a mapării: paginile mapării vor permite accese în citire și în scriere
        MAP_SHARED,
        // Maparea este partajată (altfel, ca mapare privată, nu se salvează nimic în fișierul de pe disc, la final)
        fd, // Descriptorul de fișier, din care se citesc octeții corespunzători porțiunii mapate în memorie
        pa_offset
        // Offset-ul (deplasamentul) aliniat (i.e. ajustat page-aligned), de la care începe porțiunea de fișier mapată
    ...
    fprintf(
        stderr,
        "Notification: \tthe mapping corresponds to the file's content starting at the aligned offset: %ld, and with the length: %ld\n",
        pa_offset, adjusted_length);

//      adjusted_length / PageSize gives the number of full pages.
//      adjusted_length % PageSize checks if there are leftover bytes that don't fill a full page.
//      If there are leftover bytes (% PageSize is not zero), it adds 1 more page to cover them.
//  So, it rounds up the division to ensure all bytes fit into pages. This is equivalent to ceiling division.
    long mmap_size_in_pages = (adjusted_length % PageSize ? 1 : 0) + (adjusted_length / PageSize);
    // Notă: și aici am fi putut folosi operatori pe biți!
    ...
    int i=0;
    // for (i = 0; i < 9; i++) { *(map_addr + i) = 'A' + i; } //AICI PETER GRIFFIN SCRIE COD CU PICIOARELE
    // Overwrite the first 9 bytes of the mapped region with 'A' to 'I'
    for (i = 0; i < 9; i++) {
        map_addr[i] = 'A' + i;
    }
    ...
    /* De data aceasta, maparea fiind de tip shared, veți observa că fișierul este actualizat la final pe disc!
       Notă: chiar și așa, tot mai este nevoie de folosit și apelul msync(), pentru a fi siguri că scrierea pe disc se realizează înainte de "ștergerea" mapării.
